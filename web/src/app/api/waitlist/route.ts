...
export async function POST(req: Request) {
  ...
  try {
    ...
    const notifyEmail = process.env.RESEND_NOTIFICATION_EMAIL || 'kushagra@allel.co';

    if (resendApiKey) {
      const resend = new Resend(resendApiKey);

      // 1. Send Notification Email to Founder (You)
      await resend.emails.send({
        from: 'Allel Waitlist <kushagra@allel.co>',
        to: [notifyEmail],
        subject: `🚀 New Waitlist Signup: ${email}`,
        html: `
          <div style="font-family: sans-serif; padding: 20px; background-color: #0f0f11; color: #ffffff; border-radius: 8px;">
            <h2 style="color: #4e47fa; margin-top: 0;">New Waitlist Submission!</h2>
            <p><strong>Email:</strong> <a href="mailto:${email}" style="color: #3ecf8e;">${email}</a></p>
