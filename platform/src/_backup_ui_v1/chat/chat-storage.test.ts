...
import { buildTrustedMessageMetadata } from '@/agent/tools/ui-message-utils'
import { buildAgentChatStorageScope } from '@/agent/memory/chat-session'
import {
...
test('B8: signature failure is reported rather than silently dropping history', async () => {
  const { sanitizeClientUiMessagesWithOutcome } = await import('@/agent/tools/ui-message-utils')
